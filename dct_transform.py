...
def dct_analysis(image, desired_size, noise_levels):
    # Set the desired size for DCT analysis
    sz = desired_size
    ...
    # Perform DCT
    dct_result = cv2.dct(np.float32(original_image))

    # Calculate magnitude spectrum (logarithmic scale for better visualization)
    magnitude_spectrum = np.log(np.abs(dct_result) + 1)

    # Perform Inverse DCT
    inverse_dct = cv2.idct(dct_result)

    # Display the original image, DCT magnitude spectrum, and the reconstructed image
    plt.subplot(131), plt.imshow(original_image, cmap='viridis')
    plt.title('Original Image'), plt.xticks([]), plt.yticks([])

    plt.subplot(132), plt.imshow(magnitude_spectrum, cmap='viridis')
    plt.title('DCT Magnitude Spectrum'), plt.xticks([]), plt.yticks([])

    plt.subplot(133), plt.imshow(inverse_dct, cmap='viridis')
    ...
    for noise_level in noise_levels:
        ...
        m_with_noise = dct_result + n * noise_level

        # Reconstruct the image with noise using DCT
        recon = cv2.idct(m_with_noise)

        ssim_score_dct = ssim(original_image, recon, data_range=original_image.max() - original_image.min())
        print(f"Noise Level: {noise_level}, SSIM (DCT): {ssim_score_dct}")
        ssims_dct.append(ssim_score_dct)
        ...
        print(f"Noise Level: {noise_level}, SSIM (Untransformed): {ssim_score_normal}")
        ssims_nor.append(ssim_score_normal)

        # Display the reconstructed image for each noise level
        plt.imshow(recon, cmap='viridis')
        plt.colorbar()
        plt.title(f'Noise Level: {noise_level}, SSIM (DCT): {ssim_score_dct}')
        plt.show()

    plt.plot(noise_levels, ssims_dct, label='DCT')
    plt.plot(noise_levels, ssims_nor, label='Untransformed')
    plt.title('DCT vs Untransformed SSIM')
    plt.xlabel('Noise')
    plt.ylabel('SSIM')
    plt.legend()
    plt.show()
...
if __name__ == "__main__":
    ...
    # Perform DCT analysis with additional noise and SSIM analysis
    dct_analysis(df_reduced.to_numpy(), desired_size, [0, 0.1, 0.5, 1, 2, 4, 5, 10, 15, 20])
